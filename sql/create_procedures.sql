/* PROCEDURES */
CREATE OR REPLACE PROCEDURE DELETE_ATTENDEE_FROM_CONF_DAY(
i_attendee_ID IN ATTENDEES.ATTENDEE_ID%TYPE,
i_conf_day_ID IN CONFERENCE_DAYS.CONF_DAY_ID%TYPE) AS
BEGIN
  IF NOT CHECK_ATTENDEE_ON_CONF_DAY_EXISTS(i_attendee_ID,i_conf_day_ID) THEN
  RAISE_APPLICATION_ERROR(-20400, 'Wrong Attendee on conference day!');
  ELSE
  DELETE
  FROM PARTICIPATION_CONF_DAYS
  WHERE i_conf_day_ID = CONF_DAY_ID
    AND i_attendee_ID = ATTENDEE_ID;
END IF;
END;

CREATE OR REPLACE PROCEDURE DELETE_ATTENDEE_FROM_WORKSHOP(
i_attendee_ID IN ATTENDEES.ATTENDEE_ID%TYPE,
i_workshop_ID IN WORKSHOPS.WORKSHOP_ID%TYPE) AS
BEGIN
  IF NOT CHECK_ATTENDEE_ON_WORKSHOP_EXISTS(i_attendee_ID,i_workshop_ID) THEN
    RAISE_APPLICATION_ERROR(-20400, 'Wrong ateendee on workshop!');
  ELSE
  DELETE
  FROM PARTICIPATION_WORKSHOPS
  WHERE PARTICIPATION_CONF_DAY_ID = (
    SELECT PARTICIPATION_CONF_DAYS.PARTICIPATION_CONF_DAY_ID
    FROM PARTICIPATION_CONF_DAYS
    WHERE ATTENDEE_ID = i_attendee_ID)
    AND WORKSHOP_ID = i_workshop_ID;
END IF;
END;
...
CREATE OR REPLACE FUNCTION CHECK_ATTENDEE_ON_WORKSHOP_EXISTS(
i_attendee_ID IN ATTENDEES.ATTENDEE_ID%TYPE,
i_workshop_ID IN WORKSHOPS.WORKSHOP_ID%TYPE)
RETURN BOOLEAN AS
l_attendee_on_workshop_count INTEGER;
BEGIN
  BEGIN
    SELECT COUNT(*) INTO l_attendee_on_workshop_count
    FROM PARTICIPATION_WORKSHOPS
    WHERE WORKSHOP_ID = i_workshop_ID
      AND PARTICIPATION_CONF_DAY_ID = (SELECT PARTICIPATION_CONF_DAY_ID
                                       FROM PARTICIPATION_CONF_DAYS
                                       WHERE ATTENDEE_ID = i_attendee_ID);
  END;
  RETURN (l_attendee_on_workshop_count > 0);
END

CREATE OR REPLACE FUNCTION CHECK_ATTENDEE_ON_CONF_DAY_EXISTS(
i_attendee_ID IN ATTENDEES.ATTENDEE_ID%TYPE,
i_conf_day_ID IN CONFERENCE_DAYS.CONF_DAY_ID%TYPE)
RETURN BOOLEAN AS
l_attendee_on_conf_day_count INTEGER;
BEGIN
  BEGIN
    SELECT COUNT(*) INTO l_attendee_on_conf_day_count
    FROM PARTICIPATION_CONF_DAYS
    WHERE CONF_DAY_ID = i_conf_day_ID AND ATTENDEE_ID = i_attendee_ID;
  END;
  RETURN (l_attendee_on_conf_day_count > 0);
END;
