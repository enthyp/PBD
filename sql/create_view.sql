...
//Lista klientow ktorzy nie dodali danych uczestnikiem do 14 przed konferencja//
CREATE VIEW Not_Completed_Information_About_ATTENDEE_V AS
SELECT CLIENT_NAME
FROM CLIENTS
       ...
       JOIN CONFERENCE_DAYS on CONFERENCE_DAYS.CONF_DAY_ID = PARTICIPATION_CONF_DAYS.CONF_DAY_ID
WHERE ATTENDEE_ID IN (SELECT ATTENDEE_ID FROM ATTENDEES WHERE (FIRST_NAME IS NULL OR LAST_NAME IS NULL))
