...
Add general client stats --
CREATE VIEW Client_Statistic_View_V AS
SELECT CLIENT_NAME,
       COUNT(*)                                     AS NumberOfBooking,
       SUM(VALUE)                                   AS Total_Payment,
       AVG(VaLUE)                                   AS Average_Payment,
       COUNT(WORKSHOP_BOOKINGS.NUMBER_OF_ATTENDEES) AS Number_Of_Attendees_On_Workshop,
       COUNT(CONF_DAY_BOOKINGS.NUMBER_OF_ATTENDEES) AS Number_Of_Attendees_On_Conference

FROM CLIENTS
       JOIN BOOKINGS ON CLIENTS.CLIENT_ID = BOOKINGS.CLIENT_ID
       JOIN PAYMENTS ON BOOKINGS.BOOKING_ID = PAYMENTS.BOOKING_ID
       JOIN CONF_DAY_BOOKINGS ON BOOKINGS.BOOKING_ID = CONF_DAY_BOOKINGS.BOOKING_ID
       JOIN WORKSHOP_BOOKINGS ON CONF_DAY_BOOKINGS.CONF_DAY_BOOKING_ID = WORKSHOP_BOOKINGS.CONF_DAY_BOOKING_ID
GROUP BY BOOKINGS.CLIENT_ID,BOOKINGS.BOOKING_ID, WORKSHOP_BOOKINGS.CONF_DAY_BOOKING_ID, CLIENT_NAME
