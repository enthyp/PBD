/* PROCEDURES */
CREATE OR REPLACE PROCEDURE SHOW_CONF_DAY_ATTENDANCE_LIST(
  i_conf_day_ID IN CONFERENCE_DAYS.CONF_DAY_ID%TYPE
) AS
BEGIN
  IF NOT CHECK_CONF_DAY_EXISTS(i_conf_day_ID) THEN
    RAISE_APPLICATION_ERROR(-20400, 'Wrong conference_day_ID!');
  ELSE
    SELECT *
    FROM PARTICIPATION_CONFERENCE_DAYS_V
    WHERE i_conf_day_ID = CONF_DAY_ID;
  END IF;
END;

CREATE OR REPLACE PROCEDURE SHOW_WORKSHOP_ATTENDANCE_LIST(
...
BEGIN
  ...
  ELSE
    SELECT *
    FROM PARTICIPATION_WORKSHOPS_V
    WHERE i_workshop_ID = WORKSHOP_ID;
  END IF;
END;
